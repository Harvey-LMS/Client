      ...
      >
         <div className={` p-1`}>
            <div className={`relative h-fit w-full`}>
               <div
                  className={`w-full flex flex-row justify-center items-center rounded-lg font-bold text-xl ${
                     expanded ? 'py-2 px-2' : 'p-2 hover:bg-hover'
                  }`}
               >
                  <Link
                     href={'/'}
                     className={`left-0 w-full flex flex-row items-center justify-center gap-3`}
                  >
               ...
               {expanded ? (
                  <MdNavigateBefore
                     className="absolute m-auto bottom-0 top-0 -right-3 text-foreground bg-primary-500 rounded-full"
                     onClick={() => {
         ...
         <div className="flex flex-row justify-center items-center w-full h-2">
            {expanded ? (
               <div className="flex flex-row justify-center items-center gap-1 text-hover-2 w-full">
                  <div className="bg-hover-2 h-0.5 rounded-full w-1/5"></div>
                  <p className="font-bold text-sm">Service</p>
                  <div className="bg-hover-2 h-0.5 rounded-full w-full"></div>
               </div>
            ) : (
               <div className="bg-hover-2 h-0.5 rounded-full w-full"></div>
            )}
   ...
   return (
      <div className={``}>
         <div className={` w-full flex flex-row justify-center items-center rounded-lg text-md`}>
            <Link
               href={''}
               className={`w-full h-full flex flex-row items-center bg-background ${
                  expanded ? 'px-5' : ''
               }`}
            >
               <Button
                  className={`font-semibold min-w-0 justify-start bg-background data-[hover=true]:opacity-100 hover:bg-primary hover:text-primary-foreground w-full ${
                     expanded ? '' : 'rounded-none px-7'
                  }`}
               >
         ...
         <div className="relative p-3">
            {url ? (
               <Link href={url} className="">
                  <Button
                     className={` data-[hover=true]:opacity-100 hover:text-primary-foreground z-50 text-background-foreground bg-background my-2 w-full flex flex-row min-w-0 justify-center items-center rounded-lg font-bold text-xl hover:bg-primary ${
                        expanded ? '' : 'p-2'
                     }`}
                     onClick={() => {
                        setIsDown(!isDown);
                  ...
                  >
                     <div
                        className={`w-full h-full flex flex-row items-center  ${
                           expanded ? 'py-2 px-2 justify-start gap-3' : 'm-auto justify-center'
                        }`}
                     >
                        {icon}
            ...
            <motion.div
               className={`${
                  expanded ? '' : `${isDown ? 'border absolute  left-full  top-3 m-0 ' : ''}`
               }`}
